...
function buildSystemPrompt(projects, today) {
  ...
  return [
    ...
    'priority определяй только по словам в записи:',
    '- high — есть «срочно», «горит», «немедленно», «прямо сейчас»;',
    '- normal — есть «важно», «важная», «приоритетно», но нет слов про срочность;',
    '- low — во всех остальных случаях, в том числе когда важность не названа.',
    'Не додумывай важность по смыслу задачи.',
  ].join('\n');
}
